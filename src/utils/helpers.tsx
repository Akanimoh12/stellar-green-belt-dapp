...
// ─── Reward calculation helpers (Green Belt) ───────────────────────────

/** Calculate SVT reward from XLM deposit amount (in stroops) and rate in basis points */
export function calculateReward(amountStroops: number, rateBps: number): number {
  if (amountStroops <= 0 || rateBps <= 0) return 0;
  return Math.floor(amountStroops * rateBps / 10000);
}
