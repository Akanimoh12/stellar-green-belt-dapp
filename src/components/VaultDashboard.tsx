...
import { FiLock, FiUnlock, FiUsers, FiDatabase, FiRefreshCw, FiClock, FiAward } from 'react-icons/fi';
import { useWallet } from '../hooks/useWallet';
import { useVault } from '../hooks/useVault';
import { useToken } from '../hooks/useToken';
import { formatXLM, truncateAddress, timeUntil, formatDate, Skeleton } from '../utils/helpers';
import { REWARD_RATE_BPS } from '../config/network';
...
const VaultDashboard: React.FC = () => {
  ...
  const { svtBalance, rewardInfo, loading: tokenLoading } = useToken(
    wallet?.publicKey || null
  );
  ...
  return (
    <div>
      ...
      {/* Global stats */}
      <div className="vault-stats vault-stats-4">
        ...
        <div className="stat-card stat-card-accent">
          <div className="stat-icon"><FiAward size={20} /></div>
          {tokenLoading ? (
            <Skeleton width="50%" height="1.4rem" />
          ) : (
            <div className="stat-value">{rewardInfo ? (rewardInfo.rewardRate / 100).toFixed(1) : (REWARD_RATE_BPS / 100).toFixed(1)}%</div>
          )}
          <div className="stat-label">SVT Reward Rate</div>
        </div>
        <div className="stat-card">
      ...
      {wallet && (
        <div className="user-vault-card">
          ...
          <div className="uv-row uv-row-highlight">
            <span className="uv-label"><FiAward size={13} /> SVT:</span>
            {tokenLoading ? (
              <Skeleton width="6rem" height="1rem" />
            ) : (
