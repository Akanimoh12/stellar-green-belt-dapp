...
import { FiArrowDownCircle, FiCheckCircle, FiExternalLink, FiAward } from 'react-icons/fi';
    ...
    <div>
      ...
      {result?.success ? (
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* SVT Reward Preview (Green Belt feature) */}
          {amount && parseFloat(amount) > 0 && (
            <div className="reward-preview">
              <FiAward size={16} />
              <span>
                You'll earn <strong>{(parseFloat(amount) * REWARD_RATE_BPS / 10000).toFixed(2)} SVT</strong> reward tokens
                ({(REWARD_RATE_BPS / 100).toFixed(1)}% rate)
              </span>
            </div>
          )}
