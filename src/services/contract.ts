...
import { NETWORK, CONTRACT_ID, VAULT_ADMIN, SVT_TOKEN_ID } from '../config/network';
import { VaultInfo, UserVault, AppError, AppErrorType, SvtTokenInfo, RewardInfo } from '../types';
...
export async function deposit(publicKey: string, amount: number): Promise<string> {
  ...
  // Invalidate caches after mutation (including SVT rewards)
  cache.invalidate('vault_info');
  cache.invalidate(`user_${publicKey.slice(0, 8)}`);
  cache.invalidate('svt_info');
  cache.invalidate(`svt_${publicKey.slice(0, 8)}`);
...
// ─── SVT Token read operations (Green Belt: inter-contract) ─────────

function getSvtContract(): StellarSdk.Contract {
  return new Contract(SVT_TOKEN_ID);
}

export async function getSvtTokenInfo(): Promise<SvtTokenInfo> {
  const cached = cache.get<SvtTokenInfo>('svt_info');
  if (cached) return cached;

  const server = getSorobanServer();
  const svt = getSvtContract();
  const account = await server.getAccount(VAULT_ADMIN);
  ...
  const supplyTx = new TransactionBuilder(account, {
    ...
    .addOperation(svt.call('total_supply'))
    .setTimeout(30)
    .build();

  let totalSupply = 0;
  const supplySim = await server.simulateTransaction(supplyTx);
  if (!StellarSdk.SorobanRpc.Api.isSimulationError(supplySim)) {
    const s = supplySim as StellarSdk.SorobanRpc.Api.SimulateTransactionSuccessResponse;
    if (s.result) totalSupply = Number(scValToNative(s.result.retval));
  }

  const data: SvtTokenInfo = {
  ...
  cache.set('svt_info', data, 20_000);
  return data;
}

export async function getSvtBalance(publicKey: string): Promise<number> {
  const cacheKey = `svt_${publicKey.slice(0, 8)}`;
  ...
  const tx = new TransactionBuilder(account, {
    ...
    .addOperation(svt.call('balance', new Address(publicKey).toScVal()))
    .setTimeout(30)
    .build();
  ...
  if (!StellarSdk.SorobanRpc.Api.isSimulationError(sim)) {
    const s = sim as StellarSdk.SorobanRpc.Api.SimulateTransactionSuccessResponse;
    if (s.result) balance = Number(scValToNative(s.result.retval));
  }

  cache.set(cacheKey, balance, 15_000);
  return balance;
}

export async function getRewardInfo(): Promise<RewardInfo> {
  const cached = cache.get<RewardInfo>('reward_info');
  if (cached) return cached;
  ...
  const tx = new TransactionBuilder(account, {
    ...
    .addOperation(contract.call('get_reward_info'))
    .setTimeout(30)
    .build();

  const sim = await server.simulateTransaction(tx);
  let data: RewardInfo = { rewardToken: SVT_TOKEN_ID, rewardRate: 500 };

  if (!StellarSdk.SorobanRpc.Api.isSimulationError(sim)) {
    const s = sim as StellarSdk.SorobanRpc.Api.SimulateTransactionSuccessResponse;
    if (s.result) {
      const raw = scValToNative(s.result.retval);
      data = {
        rewardToken: raw.reward_token?.toString() || SVT_TOKEN_ID,
        rewardRate: Number(raw.reward_rate || 500),
      };
    }
  }

  cache.set('reward_info', data, 60_000);
  return data;
}
