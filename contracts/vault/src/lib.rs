...
pub enum DataKey {
    ...
    TotalDeposited,
    DepositCount,
    RewardToken,     // Address of SVT token contract
    RewardRate,      // basis points: 100 = 1% reward per deposit
}
...
impl VaultContract {
    ...
    pub fn deposit(env: Env, user: Address, amount: i128) {
        ...
        env.events()
            .publish((symbol_short!("deposit"),), (user.clone(), amount, total));

        // ── Inter-contract call: Mint SVT reward tokens ──
        // If a reward token is configured, mint reward = amount * rate / 10000
        ...
        if let Some(rt) = reward_token {
            let rate: u32 = env
                ...
                .unwrap_or(0);
            if rate > 0 {
                let reward = amount * (rate as i128) / 10_000;
                if reward > 0 {
                    // Cross-contract call to SVT token contract's mint function
                    let mint_fn = Symbol::new(&env, "mint");
                    env.invoke_contract::<()>(
                        &rt,
                        &mint_fn,
                        (user.clone(), reward).into_val(&env),
                    );

                    env.events()
                        .publish((symbol_short!("reward"),), (user, reward));
                }
            }
        }
    }
    ...
    // ─── Reward token (inter-contract) functions ────────────

    /// Configure the reward token (SVT) and rate. Admin only.
    /// rate is in basis points: 500 = 5% reward per deposit.
    pub fn set_reward_config(env: Env, reward_token: Address, rate: u32) {
        let admin: Address = env.storage().instance().get(&DataKey::Admin).unwrap();
        admin.require_auth();

        if rate > 10_000 {
            panic!("rate cannot exceed 10000 (100%)");
        }
        ...
        env.storage()
            .instance()
            .set(&DataKey::RewardRate, &rate);

        env.events()
            .publish((symbol_short!("rwdcfg"),), (reward_token, rate));
    }

    /// Read reward configuration.
    pub fn get_reward_info(env: Env) -> RewardInfo {
        RewardInfo {
            reward_token: env
                .storage()
                .instance()
                .get(&DataKey::RewardToken)
                .unwrap_or(env.current_contract_address()), // fallback
            reward_rate: env
                .storage()
                .instance()
                .get(&DataKey::RewardRate)
